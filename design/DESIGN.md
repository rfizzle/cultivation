...
**Icon (`art/icon-128.png`, from `art/glyphs/icon-128.glyph`):** The bound wheat sheaf isolated — three fanned wheat heads over a twine band, stalks flaring to a flat-cut base, a small leaf sprig at each foot — with a faint warm glow, on a transparent background. Authored at 32px native and integer-laddered to the 128 and 512 masters.
...
**Icon:** not Gemini-generated — the mod icon is a glyph-pipeline master (`art/glyphs/icon-128.glyph`, 32px native, integer-laddered to the 128 and 512 masters), so its source of truth is the committed spec, not a prompt.
