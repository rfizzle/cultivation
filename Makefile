...
help:
	...
	@echo "  coverage     Run unit tests + gametests and build the merged coverage report"
...
jar: build
	@ls -1 build/libs/cultivation-*.jar 2>/dev/null | grep -Ev -- '-(sources|dev|javadoc)\.jar$$' | head -1

test:
	$(GRADLE) test

# Merged unit + gametest coverage over src/main — the mod's real coverage number.
# Report lands in build/reports/jacoco/jacocoMergedReport/ (HTML + XML).
coverage:
